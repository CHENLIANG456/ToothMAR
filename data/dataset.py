...
class InpaintDataset(data.Dataset):
    ...
    def get_mask(self):
        if self.mask_mode == "bbox":
            ...
            img_path = self.imgs[self.current_index]
            img_name = os.path.basename(img_path)
            img_name_without_ext = os.path.splitext(img_name)[0]
            
            # Construct mask path - looking for img_name_without_ext + "_mask.png"
            mask_name = f"{img_name_without_ext}_mask.png"
...
class ColorizationDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = self.tfs(
            self.loader("{}/{}/{}".format(self.data_root, "color", file_name))
        )
        cond_image = self.tfs(
            self.loader("{}/{}/{}".format(self.data_root, "gray", file_name))
        )
