...
def define_dataloader(logger, opt):
    ...
    if opt["global_rank"] == 0 and val_dataset is not None:
        dataloader_args.update(
            opt["datasets"][opt["phase"]]["dataloader"].get("val_args", {})
        )
        val_dataloader = DataLoader(
            val_dataset, worker_init_fn=worker_init_fn, **dataloader_args
        )
    else:
        val_dataloader = None
