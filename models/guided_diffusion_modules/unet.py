...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv: bool, out_channel=None):
        super().__init__()
        self.channels = channels
        self.out_channel = out_channel or channels
        self.use_conv = use_conv
...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        if up:
            ...
            self.h_upd = self.x_upd = nn.Identity()  # 不进行任何操作
    ...
    def _forward(self, x, emb):
        if self.updown:
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]  # 获取除了最后一层的归一化层和激活层
            h = in_rest(x)
            h = self.h_upd(h)
            x = self.x_upd(x)
            h = in_conv(h)
